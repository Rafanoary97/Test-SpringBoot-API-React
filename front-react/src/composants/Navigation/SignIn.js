        ...
        fetch("http://localhost:8080/users/add",
            {
                ...
                body: JSON.stringify({name:this.state.name,email:this.state.email,password:this.state.psw,session:false })
            })
            .then(response => {
            ...
            <div className="contact-clean">
                <form >
                    ...
                    <div className="form-group col d-flex justify-content-center">
                        <button className="btn btn-primary text-center" type="button" onClick={this.submitSign}>send</button>
                    </div>
                </form>
                <PiedNav/>
            </div>
